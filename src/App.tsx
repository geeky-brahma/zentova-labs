...
const Navbar = () => {
  ...
  const NavLink = ({ to, children, isAnchor = false }: { to: string, children: React.ReactNode, isAnchor?: boolean }) => {
    if (isAnchor) {
      return (
        <a 
          href={isHome ? to : `/${to}`} 
  ...
  return (
    <nav className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4">
      <motion.div 
        ...
        className="glass rounded-full px-6 py-3 flex items-center gap-8 max-w-4xl w-full justify-between border border-slate-400/20"
      >
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <NavLink to="#portfolio" isAnchor>Portfolio</NavLink>
          <NavLink to="#process" isAnchor>Process</NavLink>
          <NavLink to="#faq" isAnchor>FAQ</NavLink>
        </div>
        <a href="#contact" className="button-gradient text-background-dark px-5 py-2 rounded-full text-sm font-bold hover:opacity-90 transition-opacity text-[#0F1115]">
          Get Started
        </a>
      </motion.div>
    </nav>
...
const Hero = () => {
  ...
  const companies = [
    "TechFlow", "OmniNode", "Aura Logistics", "Vitalis", "Nexus AI", "Stellar Systems", 
    "Quantum Labs", "Horizon Data", "Apex Ventures", "CloudScale"
  ];

  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center pt-20 px-4 grid-bg overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row gap-4 mt-4">
          <Link to="/services" className="button-gradient text-[#0F1115] px-8 py-4 rounded-xl font-bold text-lg hover:scale-[1.02] transition-transform">
            View Solutions
          </Link>
          <button 
      ...
      <div className="absolute bottom-12 left-0 right-0 z-10">
        <div className="max-w-7xl mx-auto px-4">
          <p className="text-center text-slate-600 text-xs font-bold uppercase tracking-[0.3em] mb-8">Trusted by industry leaders</p>
          <div className="relative flex overflow-x-hidden">
            <div className="animate-marquee flex whitespace-nowrap items-center gap-16">
              {[...companies, ...companies].map((company, idx) => (
                <span key={idx} className="text-slate-500 text-xl md:text-2xl font-black opacity-30 hover:opacity-100 transition-opacity cursor-default select-none uppercase tracking-tighter">
                  {company}
                </span>
              ))}
            </div>
          ...
          {cases.map((item, idx) => (
            ...
            >
              ...
              <div className="flex justify-between items-start">
                <div>
                  <span className="text-xs font-bold tracking-widest text-slate-500 uppercase">{item.tag}</span>
        ...
        <div className="relative flex flex-col md:flex-row gap-8">
          {steps.map((step, idx) => (
            <div key={idx} className="flex-1 relative group">
              ...
              >
                <h4 className="text-xl font-bold mb-4 text-slate-100 group-hover:text-primary transition-colors duration-300">{step.title}</h4>
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-16">
        <div className="lg:w-1/3">
          ...
          <button className="mt-8 flex items-center gap-2 text-slate-300 font-bold hover:gap-4 transition-all group">
            Contact Specialist <ExternalLink size={18} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
          </button>
        </div>
